...
- **Stats and intents start at 0** in the constructor, but `Simulation.seed(n, rng)` randomizes them on startup: age [15,50), resources [0,100), experience [0, min(age, EXPERIENCE_CAP)], intelligence/constitution/charisma [1,10], learningIntent/exerciseIntent [0,1), stealingIntent/lyingIntent [0,0.3), killingIntent [0,0.1). Education is also seeded: ages ≤17 → `isWorkingOnEd = HIGH_SCHOOL` at 70%; ages 18–24 → `isWorkingOnEd = BACHELORS` at 40%; ages 25+ → completed `education` seeded hierarchically (HS 85%, then BACHELORS 40% conditional, MASTERS 25% conditional, PHD 20% conditional). Extraction order is shuffled each tick via Fisher-Yates using the seeded RNG. See ARD 021.
...
- **`illness` is live continuous severity `[0, 1]`**: `IllnessEvent` (unconditional, fires before `GatherResourcesEvent`) rolls onset (`BASE_ILLNESS_ONSET * ageRisk / constitution`) and recovery (`BASE_ILLNESS_RECOVERY * constitution / ageRisk`) independently each tick; severity clamped after both rolls. `ageRisk = 1 + age / ILLNESS_AGE_RISK_DIVISOR` (linear, not bell-shaped — onset is monotonically increasing). See ARD 018.
...
- `Person` data model — all properties, mutable primitives, readonly collections, `happiness` getter (ARD 014: job+5/−3 for working-age only, age-group resource thresholds, children use living-parents avg, floor 0), `livingParents` getter, `ageMortalityModifier` getter (U-shaped curve, ARD 008); `jailedTicksRemaining` (countdown to freedom, default 0; decremented by LooperSingleton before EventFactory each tick, ARD 035)
- `Simulation` — `living`, `deceased`, `history`, `decadeHistory`; `getLiving()`, `indexOfLiving()`, `getRandomOther()`, `kill()`, `add()`, `recordBirth()` (ARD 033), `seed(n, rng, personTypes?)`, `snapshot()`, `regenerate()`; Gini coefficient computed per tick; `naturalResources`, `naturalResourceCeiling`, `extractionEfficiency` resource pool fields (ARD 007); cumulative `inventionFasterCount`/`inventionSlowerCount`/`inventionCeilingCount` (ARD 032); `personTypes` and `seededTypeCounts` retained for ARD-030 end-of-run reporting; `communityPool` funded by `collectTax()` each tick and jail forfeitures; `distributeWelfare()` redistributes to persons with `resources < WELFARE_THRESHOLD` or orphaned children after consumption (ARD 034)
- `LooperSingleton.start(n, ticks, seed, logger?)` — full tick loop: prints header, seeds simulation, calls `regenerate()` then runs EventFactory per person per tick, calls `snapshot()` each tick; builds and stores a `TenYearSummary` every 10 ticks (ARD 015); after the loop appends a partial-decade summary when `ticks % 10 !== 0` (ARD 031); each tick: `collectTax` before EventFactory, `jailedTicksRemaining--` (floored at 0) before EventFactory, `distributeWelfare` after EventFactory (ARD 034, ARD 035)
- `IEvent` interface
...
- `GraduationEvent` — enrollment-gated; fires when `isWorkingOnEd !== NONE` and `rng() < BASE_GRADUATION_RATE * ageModifier(age, 22, 30, 0.15)`; sets `education = isWorkingOnEd`, resets `isWorkingOnEd = NONE`, increments `intelligence` by 1. Mutually exclusive with EnrollmentEvent in the same tick. See ARD 021.
...
- `WindfallEvent` — probability-gated at factory level (`BASE_WINDFALL_RATE * ageModifier(age, 58, 20, 0.05)`); adds flat uniform draw `WINDFALL_BASE_AMOUNT + rng() * WINDFALL_VARIANCE` to `person.resources`. Flat magnitude is the counter-force to KillEvent and StealEvent's inequality-widening. No record (external luck, no victim). See ARD 028.
- `ChildbirthEvent` — unconditional in EventFactory; fires only when `isInRelationshipWith !== null` and lower-index partner (`simulation.indexOfLiving(person) <= simulation.indexOfLiving(partner)`, for deduplication); probability uses couple aggregates (`max` illness, `min` resources, `max` age, `avg` happiness) so the dedup choice never shifts fertility — `p = BASE_CHILDBIRTH_RATE * ageModifier(coupleAge, 26, 12, 0.02) * illnessFactor * resourceFactor * happinessFactor`; on birth: deducts `CHILDBIRTH_BIRTH_COST` from each parent (floored at 0), creates `new Person([person, partner])`, pushes to both parents' `hasChildren`, calls `simulation.add(child)`. Post-birth resource floor can feed the existing starvation chain — intentional cost-of-parenthood feedback. See ARD 029.
- `InventionEvent` — intelligence-scaled probability gate (`BASE_INVENTION_RATE * intelligence * ageModifier(age, 40, 45, 0.1)`); weighted random draw selects one of three outcomes: depletion-faster (`extractionEfficiency *= 1 + delta`), depletion-slower (`extractionEfficiency *= 1 - delta`), ceiling-growth (`naturalResourceCeiling += delta * ceiling`); `delta = intelligence * INVENTION_MAGNITUDE_SCALAR`; `extractionEfficiency` floored at 0.01. See ARD 007.
- `JailEvent` — replaces gather/consume cycle for jailed persons; adds flat `JAIL_GATHER_AMOUNT` and deducts flat `JAIL_CONSUMPTION_AMOUNT`; resources floored at 0; starvation illness fires when net negative (same path as ConsumptionEvent). See ARD 035.
- `EventFactory` — when `jailedTicksRemaining > 0`, returns only `[AgeEvent, IllnessEvent, JailEvent, MisfortuneEvent]`; otherwise full suite: unconditional `[AgeEvent, ExperienceEvent, IllnessEvent, GatherResourcesEvent, ConsumptionEvent, JobEvent, RelationshipEvent, ChildbirthEvent, KillEvent, MisfortuneEvent]` plus intent-gated `ExerciseEvent`, `LearnEvent`, and `StealEvent` (with resource-pressure multiplier on steal gate), plus `EnrollmentEvent`/`GraduationEvent`, plus probability-gated `WindfallEvent` and `InventionEvent`. See ARD 010, ARD 021, ARD 023, ARD 024, ARD 025, ARD 026, ARD 027, ARD 028, ARD 029, ARD 035, ARD 036.
...
- `TickSnapshot` observability: population, per-tick and cumulative death counts by cause (murder/illness/disaster/suicide/old age), `averageResources`, `resourceGini`, `averageHappiness`, `aggregateKillingIntent`, `aggregateStealingIntent`, `naturalResources`, `extractionEfficiency` and `naturalResourceCeiling` (ARD 032), per-tick and cumulative `births` (ARD 033)
- `Reporters.ts` — `buildTenYearSummary(window, endTick, startPopulation)` (accepts partial trailing windows per ARD 031), `formatDecadeSummary`, `formatSimulationHeader`, `formatEndReport(..., personTypes?, seededTypeCounts?, living?, extinctionTick?, extractionEfficiency?, inventionCounts?)` (renders `COHORT SURVIVAL` when types supplied; `SURVIVORS` when any persons live; `Reason:` line under OUTCOME; `Extinct as of Yr NNN` callout on EXTINCTION; `Inventions:` line in RESOURCES; `Births` in POPULATION + decade table), `classifyOutcome` (EXTINCTION at population=0, checked first), `explainOutcome`, `summarizeSurvivors`, `formatSurvivorSection`. All pure; no I/O. See ARD 015, ARD 016, ARD 030, ARD 031, ARD 032, ARD 033.
...
- `ReportWriter.ts` — `writeReportHTML(simulation, n, ticks, seed)`: writes `output/report-<seed>-<outcome>-<timestamp>.html` with embedded JSON data and Chart.js charts (Gini, population+births, resources, happiness, resource-pool dynamics, antisocial intent per capita, deaths by cause); EXTINCTION outcome color added. See ARD 016, ARD 031, ARD 032, ARD 033.
