...
- `ReportWriter.ts` — `writeReportHTML(simulation, n, ticks, seed)`: writes `output/report-<seed>-<outcome>-<timestamp>.html` by compiling `report-template.hbs` with Handlebars and embedding JSON data + Chart.js charts: inequality-vs-population (Gini overlaid with population — the project's core thesis view), population & mortality (population + decade death rate per 100), happiness, health & employment, age structure (median + average age), carrying-capacity & productivity (commons-fill ratio + extraction productivity), education distribution (stacked tiers), resource-pool dynamics, couples & fertility (incl. births), antisocial intent per capita, crime & punishment (thefts + jailed population), and deaths by cause (area↔bar toggle). EXTINCTION outcome color added. Murders are shown only once (deaths-by-cause), not duplicated in the crime chart. See ARD 016, ARD 031, ARD 032, ARD 033, ARD 038.
