...
**The research goal is abundance, not collapse.** The question this model exists to answer is *what produces thriving, and which levers destroy it*. Collapse is the failure mode to be explained, not the object of study. This matters for how you read a negative result: the model currently reaches THRIVING almost never, and that is at least as likely to be a defect in the instrument as a claim about the world. When a run collapses, the useful question is not "which theory does this confirm" but "what would have had to be different for this to thrive, and is the model even capable of representing it." Treat an unreachable THRIVING as a bug to hunt until proven otherwise.
...
- **Multi-dimensional outcome classification** (ARD 051): `classifyOutcome(decadeHistory, startPopulation)` reads four collapse/thrive dimensions (Tainter/Turchin/Diamond, not HANDY-specific) — population decline **from the run's peak** (not from start), inequality (final-decade Gini), wellbeing (happiness), and ecological strain (commons fill = final-decade `avgNaturalResources ÷ avgNaturalResourceCeiling`). COLLAPSE on severe peak-decline or extreme Gini; THRIVING only when all four are good (low Gini, high happiness, near peak, healthy commons — so an overshoot with an exhausted pool can no longer read as thriving); STRUGGLING on any single stress signal; STABLE otherwise; EXTINCTION (ARD 031) still first. `explainOutcome` names the driving dimension. THRIVING is rare in practice — health coincides with declining populations, stability with overshoot. This was recorded as a genuine finding rather than a threshold bug, but that reading is **untested and should not be taken on faith**: THRIVING requires all four dimensions good simultaneously, and the commons-fill dimension is strained through any overshoot, so the category may be close to definitionally unreachable. Since the project's goal is to find what produces thriving, an outcome label that can essentially never fire is a measurement problem worth attacking directly — establish whether any state of the world can trip THRIVING before treating its absence as evidence. Refines ARD 016; research in `docs/research-collapse-classification.md`. See ARD 051.
...
- `RelationshipEvent` — unconditional; formation branch fires when unpartnered: draws `getRandomOther()` first, then checks `BASE_RELATIONSHIP_RATE * (1 + charisma * RELATIONSHIP_CHARISMA_SCALAR) * ageModifier(person.age, 26, 35, 0.1) * ageModifier(|ageGap|, 0, RELATIONSHIP_AGE_GAP_SCALE, RELATIONSHIP_AGE_GAP_FLOOR)` — the age-gap modifier discounts cross-generational pairings proportionally (ARD 054); target must also be unpartnered; mutually assigns both fields. Dissolution branch fires when partnered (`BASE_BREAKUP_RATE` flat probability), mutually clears both fields. Persons below `RELATIONSHIP_MIN_AGE` skip entirely (ARD 053). `Simulation.kill()` also clears surviving partner's field on death. See ARD 025, ARD 053, ARD 054.
