...
**The research goal is abundance, not collapse.** The question this model exists to answer is *what produces thriving, and which levers destroy it*. Collapse is the failure mode to be explained, not the object of study. This matters for how you read a negative result: the model currently reaches THRIVING almost never, and that is at least as likely to be a defect in the instrument as a claim about the world. `docs/research-thriving-reachability.md` established that it *is* partly the instrument — THRIVING is reachable and sustainable at 800 ticks, and three of the four things blocking it are model defects rather than findings. When a run collapses, the useful question is not "which theory does this confirm" but "what would have had to be different for this to thrive, and is the model even capable of representing it." Treat an unreachable THRIVING as a bug to hunt until proven otherwise.
