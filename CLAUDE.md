...
- **End-of-simulation report**: After the tick loop, `index.ts` calls `formatEndReport` (console summary with outcome verdict) and `writeReportHTML` (writes `output/report-<seed>-<timestamp>.html` — self-contained HTML with Chart.js charts loaded from CDN). Outcome classification (`COLLAPSE`/`STRUGGLING`/`STABLE`/`THRIVING`) uses named threshold constants in `Variables.ts`. Population retention acts as a ceiling on upper tiers: below `THRIVING_MIN_POPULATION_FRACTION` caps at STABLE; below `STRUGGLING_MAX_POPULATION_FRACTION` forces STRUGGLING. I/O in `src/Helpers/ReportWriter.ts`, pure formatting in `Reporters.ts`. See ARD 016, ARD 031.
...
- `Variables.ts` — outcome classification thresholds: `COLLAPSE_GINI_THRESHOLD`, `COLLAPSE_POPULATION_FRACTION`, `STRUGGLING_GINI_THRESHOLD`, `STRUGGLING_HAPPINESS_THRESHOLD`, `STRUGGLING_MAX_POPULATION_FRACTION`, `THRIVING_GINI_THRESHOLD`, `THRIVING_HAPPINESS_THRESHOLD`, `THRIVING_MIN_POPULATION_FRACTION`
