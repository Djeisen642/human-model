...
- `LooperSingleton.start(n, ticks, seed, logger?)` — full tick loop: prints header, seeds simulation, calls `regenerate()` then runs EventFactory per person per tick, calls `snapshot()` each tick; builds and stores a `TenYearSummary` every 10 ticks (ARD 015); after the loop appends a partial-decade summary when `ticks % 10 !== 0` (ARD 031)
- `IEvent` interface
...
- `KillEvent` — unconditional in EventFactory (intent gate is inside `execute()` because Gini requires simulation access); attempt prob = `killingIntent * ageModifier(age, 24, 30, 0.05) * (1 + currentGini * KILL_GINI_SCALAR)`; if attempt fires, success prob = `KILL_SUCCESS_BASE / max(1, victim.constitution)`; on success calls `simulation.kill()` which creates `DeathRecord` (MURDER) and `KillingRecord`. Gini modulator is the primary collapse feedback: inequality → elevated killing → population loss → further inequality. See ARD 027.
...
- `EventFactory` — unconditional `[AgeEvent, ExperienceEvent, IllnessEvent, GatherResourcesEvent, ConsumptionEvent, JobEvent, RelationshipEvent, ChildbirthEvent, KillEvent, MisfortuneEvent]` plus intent-gated `ExerciseEvent`, `LearnEvent`, and `StealEvent` via `rng() < intent * ageModifier(...)`, plus `EnrollmentEvent` and `GraduationEvent` (mutually exclusive by condition), plus probability-gated `WindfallEvent` and `InventionEvent`. See ARD 010, ARD 021, ARD 023, ARD 024, ARD 025, ARD 026, ARD 027, ARD 028, ARD 029.
