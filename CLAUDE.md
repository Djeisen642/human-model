...
- **Progress reporting**: `LooperSingleton` prints a one-line decade summary to `console.log` each time a `TenYearSummary` is produced. Formatting lives in `src/Helpers/Reporters.ts` (pure functions, independently testable). See ARD 016.
- **End-of-simulation report**: After the tick loop completes, `index.ts` calls `formatEndReport` (console) and `writeReportJSON` (writes `output/report-<seed>-<timestamp>.json`). Includes outcome classification (`COLLAPSE`/`STRUGGLING`/`STABLE`/`THRIVING`) with named threshold constants in `Variables.ts`. I/O lives in `src/Helpers/ReportWriter.ts`, separate from pure formatting. See ARD 017.
...
- `SeededRandom` (LCG), `RNG` type, `Constants`, `Variables` (includes `PRIME_AGE`, `AGE_DEATH_CURVATURE`, and per-event age profile constants for all planned events)
