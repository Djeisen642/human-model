...
- `LooperSingleton.start(n, ticks, seed, logger?)` — full tick loop: prints header, seeds simulation, calls `regenerate()` then runs EventFactory per person per tick, calls `snapshot()` each tick; builds and stores a `TenYearSummary` every 10 ticks (ARD 015); after the loop appends a partial-decade summary when `ticks % 10 !== 0` (ARD 031); each tick: `collectTax` before EventFactory, `jailedTicksRemaining--` (floored at 0) + `helpHappinessBoost`/`killHappinessBoost` decay before EventFactory, `distributeWelfare` after EventFactory (ARD 034, ARD 035, ARD 046)
- `IEvent` interface
