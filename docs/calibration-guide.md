...
- **`EXTRACTION_ACCESS_GRADIENT` is a distribution dial, and only reads as one if you keep aggregate extraction fixed.** *(ARD 068.)* It scales each adult's gather potential by their wealth relative to the adult median, normalised so the adult mean multiplier is exactly 1 — so at a fixed population the arms being compared have the same total extraction capacity and differ only in who holds it. **That normalisation is the entire validity of the comparison.** Anything that breaks it (a hand-edited multiplier, comparing across arms that also move `BASE_GATHER_AMOUNT`/`INTELLIGENCE_GATHER_SCALAR`/`EXTRACTION_PRODUCTIVITY_*`) turns the result back into the extraction lever, which the bullet above establishes dominates every outcome in this model. Three things to know before sweeping it. The gradient at 0 is the exact baseline (bitwise, `scripts/parity-check.ts`), so a 0 arm is a real control and not an approximate one. The usable range is roughly `0` to `0.75`: a reduced-form model of this wealth dynamic condenses past ~0.9, where one agent takes everything and the median pins to `WELFARE_THRESHOLD`. And the effect is expected to run through redistribution, not around it — welfare already sets the floor for the bottom half in the surviving regime — so cross any gradient sweep with `TAX_RATE` at 0 and at default, or you cannot tell a null from an effect the community pool absorbed.
