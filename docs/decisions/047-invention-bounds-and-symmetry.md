...
- `docs/model-reference.md` — update the "Global natural resource pool" design bullet; `docs/odd-protocol.md` §7 — update the `InventionEvent` submodel, both to state the productivity walk is bounded `[FLOOR, MAX_EXTRACTION_PRODUCTIVITY]` with symmetric faster/slower steps, and the ceiling is capped at `MAX_NATURAL_RESOURCE_CEILING`.
