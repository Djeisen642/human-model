...
**Replace `peakGini` as the sweep harness's inequality column** — see `docs/research-gini-metric.md`
Per-tick instrumentation suggests `peakGini` is attained during the terminal crash at populations of ~10–30, making it a max-of-noise statistic that reads 0.79–0.89 for nearly every config — including configs whose actual inequality differs fourfold (`TAX_RATE` 0 vs 0.20 gives mature-phase Gini 0.574 vs 0.142 while `peakGini` reports 0.733 vs 0.714, and in one spot check ordered them backwards). It cuts both ways: re-testing found `CONSUMPTION_ELDER_MULTIPLIER`'s clean monotone `peakGini` trend (0.72→0.90) does *not* reproduce on a mature-phase median, so the column manufactures apparent effects as well as hiding real ones. If it holds, every calibration read off that column is suspect, and the fix is measurement, not parameters. Hypothesis only, and the same study puts the resolution limit at ~±0.05 `matureGini` at 10 seeds — which most single-lever effects ever measured here fall inside. Establish the noise floor and a defensible window definition first; changing the harness's headline metric is ARD-level since it redefines how every sweep is read.

**Estate shares have an unenforced sum-to-1.0 invariant** — see `docs/research-gini-metric.md`
`Simulation.kill()` applies `ESTATE_COMMUNITY_SHARE`/`ESTATE_PARTNER_SHARE`/`ESTATE_CHILDREN_SHARE` raw, with no normalisation or validation, so sweeping one alone silently breaks the resource conservation ARDs 039–042 established (0.9 with the others at defaults distributes 1.5× each estate; 0 destroys 40%). Defaults are fine, so this bites calibration runs rather than normal operation. Decide whether to normalise at use, validate at config load, or just document the invariant.
...
> **Re-verified 2026-09-14 (commit 84c675d) — the "finds none" scan was run at default productivity dynamics; a different regime finds it a lot.** `docs/research-sweep-session-2026-09-14.md` found that removing `extractionProductivity`'s variance produces sustained cycling in most seeds. The threshold-free version of the result: seeds that never oscillate at all (`cyc=0`) drop from **10/16 at default to 3/16 with productivity pinned**, with a re-measured default control confirming the baseline. 1500-tick follow-ups on both productivity configs show the regime is real but is **not an equilibrium**: 17–19% of surviving seeds go extinct per 700 ticks in both, because the cycles bottom out at a median of **10 people** (range 5–18) — roughly a 7% extinction hazard per trough, with no sign of decaying. Three caveats before anyone leans on this: `stableCycle` passes at `troughTrend ≥ 0.5`, so it tolerates troughs halving over a run and reads closer to "still oscillating" than "holds a floor"; every config that cycles well runs at 41–61% `bound%`, i.e. cycling against an exhausted commons rather than thriving; and the best-looking cell (zero extinctions at 800 ticks) went to 3/16 extinct by 1500. So this doesn't contradict the original scan (which never tried removing productivity drift), but it *does* sharpen the relationship to crash-recovery: the productivity fix produces the oscillation, and crash-recovery is what would make it survivable. **Concrete target for that ARD: the mechanism must bite at populations of 5–20, and trough depth — not `stable` counts — is the calibration metric.** Promoting the `OSCILLATING` label is still ARD-level, still blocked on the per-cycle extinction hazard, and would also want a tighter `troughHoldFraction` than the current 0.5.
