...
**Statistical power of the sweep metrics**
`stable` counts drove this project's main conclusions but their noise floor is unknown. A 2026-09-13 session twice inferred a curve shape from 8 seeds that dissolved at 16 and 32, and its decisive result (3/32 vs 0/32) sits at Fisher p ≈ 0.24. Run one config at 32/64/128 seeds and report the variance of `stable`, `extinct`, and peak population, to establish how many seeds an effect of a given size actually needs. Companion to the `peakGini` work in `docs/research-gini-metric.md`, which already showed one headline column to be a max-of-noise statistic. Pure measurement, no ARD needed, and it makes every later study cheaper and more honest.
