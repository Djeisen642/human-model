...
> **Re-verified 2026-09-14 (commit 84c675d) — the "finds none" scan was run at default productivity dynamics; a different regime finds it a lot.** `docs/research-sweep-session-2026-09-14.md` found that removing `extractionProductivity`'s variance produces sustained cycling in most seeds. The threshold-free version of the result: seeds that never oscillate at all (`cyc=0`) drop from **10/16 at default to 3/16 with productivity pinned**, with a re-measured default control confirming the baseline. A 1500-tick follow-up on the pinned config showed this is real but incomplete — 7 of 10 seeds tagged `STABLE-CYCLE` at 800 ticks were still cycling at 1500 (three rebuilt to 61–97% of their historical peak), but 2 rolled an extinction on a later down-phase anyway. Two caveats before anyone leans on this: `stableCycle` passes at `troughTrend ≥ 0.5`, so it tolerates troughs halving over a run and is closer to "still oscillating" than "holds a floor"; and every config that cycles well runs at 41–61% `bound%`, i.e. it is cycling against an exhausted commons rather than thriving. So this doesn't contradict the original scan (which never tried removing productivity drift), but "premature until crash-recovery exists" needs a caveat: some of the gap may close from fixing a calibration bug rather than needing the new mechanism. Promoting the label is still ARD-level, still blocked on the residual extinction risk, and would also want a tighter `troughHoldFraction` than the current 0.5.
