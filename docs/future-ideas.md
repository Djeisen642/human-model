...
Per-tick instrumentation suggests `peakGini` is attained during the terminal crash at populations of ~10–30, making it a max-of-noise statistic that reads 0.79–0.89 for nearly every config — including configs whose actual inequality differs fourfold (`TAX_RATE` 0 vs 0.20 gives mature-phase Gini 0.574 vs 0.142 while `peakGini` reports 0.733 vs 0.714, and in one spot check ordered them backwards). If it holds, every calibration read off that column is suspect, and the fix is measurement, not parameters. Hypothesis only: 8–10 seeds, unreplicated, and the proposed `matureGini` cutoff (pop ≥ 50% of run peak) is an unjustified judgment call. Needs replication + a cutoff sensitivity check first; changing the harness's headline metric is ARD-level since it redefines how every sweep is read.
