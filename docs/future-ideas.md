...
> **Re-verified 2026-09-14 (commit 84c675d) — the "finds none" scan was run at default productivity dynamics; a different regime finds it a lot.** `docs/research-sweep-session-2026-09-14.md` found that removing `extractionProductivity`'s downward drift (fully pinning it, or pairing a wider band with a higher invention rate) produces sustained cycling in most seeds — `stable` up to 15/16 at 800 ticks (vs. 2/32 default). A 1500-tick follow-up on the full-pin config showed this is real but incomplete: 7 of 10 seeds tagged `STABLE-CYCLE` at 800 ticks were still cycling at 1500 (three rebuilt to 61–97% of their historical peak), but 2 of the 10 rolled an extinction on a later down-phase anyway — so the underlying dynamics are most of the way to a genuine oscillating regime, but not a hard floor yet. This doesn't contradict the original scan (which never tried removing productivity drift), but it means "premature until crash-recovery exists" needs a caveat: a large chunk of the gap may already close from fixing a measurement/calibration bug rather than needing the new mechanism. Promoting the label is still ARD-level and still blocked on deciding how to handle the residual extinction risk, but the productivity-drift fix (see the item above) is now the more concrete prerequisite to test first.
