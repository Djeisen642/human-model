...
**Resource Gini counts dependent children's structural zeros** — surfaced by ARD 056/057
The Gini (`Simulation.snapshot()`) is computed over *all* living persons' personal `resources`. Children are economically dependent — newborns enter at 0 (ARD 037) and ARD 057 now seeds children at 0 too — so a population's child share directly inflates the measured Gini independent of any adult inequality. With ARD 056's young-skewed age taper this is more pronounced (more children). The signal is *consistent* seed-to-runtime, but it conflates "many dependents" with "unequal adults." Worth deciding whether the collapse signal should be computed over adults (or working-age) only, or households (with resource pooling), so the headline Gini tracks adult inequality rather than dependency ratio. Measurement-only change to the signal; small ARD if adopted because it redefines the primary metric. **Now quantified, and smaller than first reported** (`docs/research-thriving-reachability.md`): across 840 decade-observations the median gap between the two bases runs +0.002 at a <5% child share to +0.086 at 25–35%, with p90 +0.092 and a maximum of +0.192 — the widely-quoted +0.15 was a single tail decade. In 12.5% of decades the adult basis reads *higher*. The pathology (equal adults failing ARD 051's 0.30 THRIVING gate on age structure alone) fires in 6.7% of decades. Worth fixing so the primary signal means what it says, but not a lever expected to move outcomes much. **Agreed and specified in ARD 060** — adults 18+, one definition across the snapshot and KillEvent, ARD-051 thresholds re-derived by quantile preservation.
