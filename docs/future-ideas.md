...
**`distributeWelfare` concentrates when few qualify** — see `docs/research-thriving-reachability.md`
The payout splits `communityPool × (1 − reserve)` **equally among the eligible**, so in a wealthy population where almost nobody is below `WELFARE_THRESHOLD` the entire tax take lands on a handful of people. Observed: with `TAX_RATE=0.05, WELFARE_THRESHOLD=60`, Gini climbed 0.17 → 0.86 while everyone was healthy, employed, and the commons was full — the mechanism meant to compress inequality manufactured it. Fix options: cap the per-capita share, or make the payout proportional to each recipient's shortfall rather than an equal split of the whole pool. Interacts directly with the redistribution-calibration item below.
...
The Gini (`Simulation.snapshot()`) is computed over *all* living persons' personal `resources`. Children are economically dependent — newborns enter at 0 (ARD 037) and ARD 057 now seeds children at 0 too — so a population's child share directly inflates the measured Gini independent of any adult inequality. With ARD 056's young-skewed age taper this is more pronounced (more children). The signal is *consistent* seed-to-runtime, but it conflates "many dependents" with "unequal adults." Worth deciding whether the collapse signal should be computed over adults (or working-age) only, or households (with resource pooling), so the headline Gini tracks adult inequality rather than dependency ratio. Measurement-only change to the signal; small ARD if adopted because it redefines the primary metric. **Now quantified** (`docs/research-thriving-reachability.md`): on the default config at tick 60, `giniAll = 0.346` against `giniAdults = 0.192` at a 30% child share — roughly **+0.15 from dependency ratio alone**. That is decisive for ARD 051's THRIVING gate at 0.30: a society with genuinely equal adults fails it on age structure, and since growth raises child share, population growth mechanically pushes Gini through the gate while the peak-decline gate demands growth. Promote.
