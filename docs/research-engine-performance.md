...
**Recorded:** 2026-09-14 | **Commit:** 34bf7ff | **Latest ARD:** 062 | **Base config:** all Variables at defaults
**Commands:** ad-hoc benchmark calling `LooperSingleton.start(n, 30, 42, () => {})` for n in 400/800/1600/3200; `npx ts-node scripts/parity-check.ts --emit /tmp/wide.json --seeds 32 --ticks 300` on the unmodified revision, then `--verify` after each change
**Key context vars:** `KILL_GINI_SCALAR=1.5`, `SITUATIONAL_KILL_SCALAR=1.0`, `WORKING_AGE_MIN=18` (sets the Gini basis), `BASE_CHILDBIRTH_RATE=0.6` (sets how far population grows during the run)
...
## Population cost was quadratic

Wall time for 30 ticks, seed 42, seeded population `n`:

| n | Before | After | Speedup |
|---|---|---|---|
| 400 | 976 ms | 130 ms | 7.5× |
| 800 | 3,785 ms | 271 ms | 14× |
| 1600 | 14,855 ms | 912 ms | 16× |
| 3200 | — | 3,383 ms | — |

Before the fix, doubling the population multiplied the time by about four — the signature of an
algorithm doing work proportional to the square of the population. Extrapolated, a 10,000-person run
over 800 ticks would have taken roughly four hours, which is why that scale was out of reach.
...
So the residual Gini calls are the entire remaining curve. At n=3200 they are still 89% of runtime —
a 3% chance of an expensive call still beats a linear pass once the population is large enough.

Removing them means computing the Gini once at tick start and reading that fixed value for everyone,
which changes results and therefore needs an ARD. Two exact alternatives were considered and both fail:
...
for every tick across 32 seeds × 300 ticks and found no difference. The baseline was emitted from the
unmodified revision with the changes stashed, so it is a true before-and-after, not a self-comparison.
